...
func TestBotFeatureTogglesPreserveTransportAndConsole(t *testing.T) {
	...
	cfg.Modules = runtimeconfig.ModulesConfig{Bot: true}
	...
	m := NewManager(runtimeconfig.WithSource(ctx, runtimeconfig.NewSource(cfg)), nil, nil, Options{ComponentStore: store})
...
func TestBotComponentConfigurationPersistence(t *testing.T) {
	...
	manager := &Manager{componentStore: store, savedStore: store, botComponents: host}
	...
	require.False(t, console.Allowed(types.DefaultAccount, 7))
	...
	restarted, restored, newNotifications, err := application.BotHost(ctx, types.DefaultAccount, transport, []int64{42}, store)
	require.NoError(t, err)
	defer func() { require.NoError(t, restarted.Stop(ctx)) }()
	require.True(t, restored.Allowed(types.DefaultAccount, 7))
	_, err = newNotifications.Send(ctx, "hello after restart")
	require.NoError(t, err)
