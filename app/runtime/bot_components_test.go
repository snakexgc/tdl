...
func TestBotComponentConfigurationPersistence(t *testing.T) {
	...
	require.NoError(t, manager.SaveComponentConfiguration(ctx, consoleID, map[string]any{"allowed_users": []string{"7"}}))
	...
	require.Error(t, manager.SaveComponentConfiguration(ctx, consoleID, map[string]any{"allowed_users": []string{"invalid"}}))
	require.True(t, console.Allowed(types.DefaultAccount, 7))
