...
func (m *memoryRepository) Update(_ context.Context, id string, change func(*Job) bool) (bool, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	job, exists := m.jobs[id]
	if !exists || !change(&job) {
		return false, nil
	}
	m.jobs[id] = job
	return true, nil
...
func (m *memoryRepository) Save(_ context.Context, j Job) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *memoryRepository) Get(_ context.Context, id string) (Job, bool, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	j, ok := m.jobs[id]
	return j, ok, nil
...
func (m *memoryRepository) Records(context.Context) ([]Job, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
	out := []Job{}
...
func (m *memoryRepository) Remove(_ context.Context, id string) error {
	m.mu.Lock()
	defer m.mu.Unlock()
	delete(m.jobs, id)
	return nil
...
func newTestQueue() *Queue {
	return NewQueue(&memoryRepository{jobs: map[string]Job{}})
}
