...
func TestControllerOverviewCountsOwnedRemainingAndRetryableTasks(t *testing.T) {
	t.Parallel()
...
func TestControllerPauseStartAndRetryOnlyOwnedTasks(t *testing.T) {
	t.Parallel()
...
func TestControllerSubmitsGeneratedHTTPLink(t *testing.T) {
	t.Parallel()
	...
	result, err := controller.Submit(ctx, types.DownloadSubmission{
		TaskID:      testDocument1,
		DownloadURL: testDownloadURL1,
		Dir:         testDownloadDir,
		Out:         testVideoFilename,
	...
	require.NoError(t, err)
	require.Equal(t, types.DownloadResult{Account: types.DefaultAccount, Target: aria2DownloaderName, ID: testGIDNew}, result)
	require.Equal(t, []string{testDownloadURL1}, client.addedURIs)
...
func TestAria2SubmissionRejectsAnotherAccountBeforeRPC(t *testing.T) {
	client := &fakeAria2ControlClient{addedGID: testGIDNew}
	store := newTestRepository()
	controller := &Controller{account: testAccount, client: client, store: store, logger: zap.NewNop()}
	...
	result, err := controller.Submit(context.Background(), types.DownloadSubmission{
		Account: testAccount, TaskID: testDocument1, DownloadURL: testDownloadURL1,
	})
	require.NoError(t, err)
