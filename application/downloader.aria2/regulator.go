...
type TelegramErrorRegulator struct {
	configuration func() telegramErrorRegulatorConfig
...
func (r *TelegramErrorRegulator) settings() telegramErrorRegulatorConfig {
...
func NewTelegramErrorRegulator(client TelegramErrorRegulatorClient, store ports.Aria2Repository, publicBaseURL string, logger *zap.Logger) *TelegramErrorRegulator {
	return newTelegramErrorRegulator(client, store, publicBaseURL, logger, telegramErrorRegulatorConfig{})
}

func newTelegramErrorRegulator(client TelegramErrorRegulatorClient, store ports.Aria2Repository, publicBaseURL string, logger *zap.Logger, cfg telegramErrorRegulatorConfig) *TelegramErrorRegulator {
	if logger == nil {
		logger = zap.NewNop()
	}
...
func (r *TelegramErrorRegulator) recordError(err error) bool {
	...
	if len(r.timestamps) < cfg.Threshold {
		return false
	}
	if r.regulating {
		return false
	}
	if !r.lastAction.IsZero() && now.Sub(r.lastAction) < cfg.Cooldown {
		return false
	}
