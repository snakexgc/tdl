...
type ZeroSpeedMonitor struct {
	configuration func() zeroSpeedMonitorConfig
	changed       chan struct{}
...
func NewZeroSpeedMonitor(client ZeroSpeedMonitorClient, store ports.Aria2Repository, publicBaseURL string, logger *zap.Logger) *ZeroSpeedMonitor {
	return newZeroSpeedMonitor(client, store, publicBaseURL, logger, zeroSpeedMonitorConfig{})
}

func newZeroSpeedMonitor(client ZeroSpeedMonitorClient, store ports.Aria2Repository, publicBaseURL string, logger *zap.Logger, cfg zeroSpeedMonitorConfig) *ZeroSpeedMonitor {
	if logger == nil {
		logger = zap.NewNop()
	}
...
func (m *ZeroSpeedMonitor) settings() zeroSpeedMonitorConfig {
...
func (m *ZeroSpeedMonitor) Run(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-m.changed:
			ticker.Reset(m.settings().PollInterval)
		case <-ticker.C:
...
func (m *ZeroSpeedMonitor) poll(ctx context.Context) error {
	...
	for gid, task := range activeOwned {
		...
		if now.Sub(m.stall[gid]) >= cfg.StallThreshold {
	...
	for _, gid := range stalled {
		m.logger.Warn("Active aria2 task has had zero download speed beyond stall threshold; cycling pause/resume",
			zap.String("gid", gid),
			zap.Duration("stall_threshold", cfg.StallThreshold))
...
func (m *ZeroSpeedMonitor) pauseAndResume(ctx context.Context, gid string) error {
	...
	m.logger.Info("Paused stalled aria2 task, waiting before resume",
		zap.String("gid", gid),
		zap.Duration("pause_duration", cfg.PauseDuration))
	...
	actionCtx2, cancel2 := context.WithTimeout(ctx, cfg.ActionTimeout)
