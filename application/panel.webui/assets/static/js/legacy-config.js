...
const sections = [
  ...
  },
  {
    title: "基础",
    fields: [
      ["proxy", "代理地址", "proxy", "Telegram、机器人和软件更新共用此代理。选择协议后填写地址，例如 127.0.0.1:1080。"],
...
export async function loadConfig(event) {
  if (event && loadedSnapshot !== snapshot() && !confirm("重新读取将撤销尚未保存的旧版配置，是否继续？")) return;
  ...
  try {
    ...
    renderConfigForm();
    document.getElementById("legacy-settings")?.dispatchEvent(new Event("legacy-config-saved"));
  } catch (error) {
...
function renderConfigForm() {
  ...
  for (const section of sections) for (const field of section.fields) {
    if (field[0].startsWith("modules.")) continue;
    if (componentManaged && field[0] !== "debug") continue;
    const tab = legacyTab(field[0]);
    const key = tab + section.title;
    if (!grouped.has(key)) grouped.set(key, { title: section.title, tab, fields: [] });
    grouped.get(key).fields.push(field);
  }
  const visibleSections = [...grouped.values()];
  ...
  showTab(new URLSearchParams(location.search).get("tab") || "network");
...
function legacyTab(path) {
  if (path.startsWith("bot.notify.")) return "notifications";
...
export function showTab(tab) {
  document.querySelectorAll("[data-legacy-tab]").forEach(section => { section.hidden = section.dataset.legacyTab !== tab; });
}
export function snapshot() {
  return JSON.stringify([...document.querySelectorAll("#config-form [data-config-control]")].map(input => {
    let value = fieldValue(input, input.dataset.type);
    if (input.classList.contains("tag-input")) {
      const pending = splitList(input.querySelector("[data-tag-entry]")?.value || "");
      value = [...new Set([...tagValues(input), ...pending])];
      if (input.dataset.type === "intList") value = value.map(Number).filter(Number.isFinite);
    }
    return [input.dataset.path, value];
  }));
}
