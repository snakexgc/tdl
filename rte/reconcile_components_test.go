...
func TestReconcileComponentsPreservesUnrelatedAndRebindsOptionalPorts(t *testing.T) {
	...
	for _, id := range []string{consumerID, "unrelated"} {
	...
	build := func(enabled bool) *rte.Runtime {
		run, err := registry.Build(types.DefaultAccount, map[string]bool{providerID: enabled, consumerID: true, "unrelated": true}, nil)
	...
	require.Equal(t, 1, starts["unrelated"])
	require.Zero(t, stops["unrelated"])
