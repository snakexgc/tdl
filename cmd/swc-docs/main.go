...
func run(path string) error {
	...
	fmt.Fprint(&buffer, "WebUI 设置覆盖全部 75 个组件字段及详细日志开关。分类依次为网络连接、下载设置、转发设置、机器人设置、通知设置、系统设置。Telegram API 凭据并入系统设置，下载链接服务位于下载设置最前面，面板访问位于系统设置最前面。全部字段直接展开，支持按中文名称、说明或字段名搜索并定位；搜索不索引实际配置值。分类和区块显示未保存数量。顶部仅保留“刷新”，从 tdl_config.json 重新读取配置，替换当前未保存输入。下载方式单选本地下载器或 aria2，HTTP 服务始终启动供两种方式和复制直链使用；本地下载目录与 aria2 保存目录分别说明所属机器。字段显示默认值、范围和生效方式，输入错误在字段旁提示。账号数据空间不在设置页展示，切换入口在账号管理；完全重置位于系统设置的应用维护区。\n\n有修改时，顶部显示“尚未保存的修改”框，逐项对比当前运行值和修改后的值，并区分尚未保存与已保存待重启；没有修改时隐藏整个框及按钮。框内“保存并重启”一次校验并保存所有分类的草稿，然后请求重启；也可用区块按钮先保存文件。校验或保存失败不重启，未保存输入保留，已成功写入文件的区块会单独标记。普通字段及允许操作机器人的用户列表直接填入输入框，代理和 aria2 地址也将去掉认证信息后的地址填入输入框，不在下方重复显示已保存值；地址未修改时保留完整配置，修改地址时需同时填写所需的认证信息；API Hash、Token、密码和 RPC 密钥不回显。刷新页面仍可看到文件与运行值的差异，改回启动值并保存后移除对应项。重启会暂时中断连接和任务，新配置生效后清单清空；如修改了面板地址或凭据，请使用新地址或凭据重新登录。\n\n")
	...
	fmt.Fprint(&buffer, "WebUI 的“系统设置 → 应用维护 → 完全重置”须再次确认后执行。重置会停止服务、关闭数据库与日志，再清空应用目录内 `.tdl/` 的内容，删除 `tdl_config.json` 及其写入临时文件，包含全局配置、所有账号的登录凭据和任务记录。数据目录本身保留以支持挂载；完成后程序退出，下次启动需重新配置与登录，容器遵循自身重启策略。清理结果显示在程序控制台。\n\n")
	fmt.Fprint(&buffer, "新配置的 Telegram 内置预设默认为 `desktop`，WebUI 提供下拉选择并提示不建议修改；“使用内置 API 凭据”默认勾选，位于系统设置的 Telegram API 凭据区块。API ID 未设置时显示空输入框，API Hash 不回显，两者留空均保留原值。内置预设必须明确选择；已有会话缺少凭据指纹时需要重新登录，不再猜测原应用。断线重试和时间校准直接显示在“网络连接”中；重连间隔必须为 1–86400 秒，默认 3 秒，0 不再作为另一套默认值的别名。\n\n")
