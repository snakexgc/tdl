...
func run(path string) error {
	...
	fmt.Fprint(&buffer, "未启动或已停用组件仍可编辑配置。标注需要重启的字段先保存，再协调所属服务；失败时运行状态保留错误，配置页显示等待重启。其他字段经准备、校验和持久化后发布。WebUI 中秘密字段省略或留空表示保留已保存的值；直接编辑 JSON 时，空值表示清空，省略则使用默认值。ntp 留空使用系统时钟，不再探测或自动写回 NTP 地址。\n\n")
	fmt.Fprint(&buffer, "网络代理统一在 WebUI 的“网络配置 → 网络代理”中配置；网络配置位于设置标签首位并默认打开，选择协议后填写 IP 或域名加端口，需要认证时展开填写用户名和密码。由 account.telegram.proxy 保存，Telegram、机器人和软件更新共用。模块启停位于独立“模块管理”；运行日志、组件健康及诊断事件位于“检查更新”之前的独立“日志管理”，支持按全部功能、功能分组和细分模块查看。软件升级位于独立“检查更新”。\n\n")
	fmt.Fprint(&buffer, "`download.control.executors` 可设置为 `[\"aria2\",\"local\",\"http\"]`，选择本地执行器时必须同时提供本机绝对路径 `local_root`。只有明确未接受任务的错误允许降级；超时、响应丢失或已有任务 ID 时停止提交。aria2 仍需启用对应模块和自动下载。列表不能为空，默认顺序为 aria2、http；更改仅影响新提交。只接受 local、aria2、http，不接受 internal 或 mode 字段；本地任务只使用 download.local 键空间，任务记录必须具有当前状态与版本字段，不自动补齐历史记录。\n\n")
	fmt.Fprint(&buffer, "下载执行器顺序、本地根目录、并发文件数、DC 连接数、过滤/命名、表情触发、HTTP 公网地址/TTL 和分组转发规则支持热更新。表中“需要重启”指自动协调对应服务及必要依赖，不等于整个进程重启。账号命名空间、存储位置和程序升级需要进程重启。\n\n")
	fmt.Fprint(&buffer, "WebUI 的“系统设置 → 一键完全重置”须再次确认后执行。重置会停止服务、关闭数据库与日志，再清空应用目录内 `.tdl/` 的内容，删除 `tdl_config.json` 及其写入临时文件，包含全局配置、所有账号的登录凭据和任务记录。数据目录本身保留以支持挂载；完成后程序退出，下次启动需重新配置与登录，容器遵循自身重启策略。清理结果显示在程序控制台。\n\n")
	fmt.Fprint(&buffer, "新配置的 Telegram 内置预设默认为 `desktop`，WebUI 提供下拉选择并提示不建议修改；“强制使用内部预设”默认勾选。API ID 未设置时显示空输入框，API Hash 不回显，两者留空均保留原值。内置预设必须明确选择；已有会话缺少凭据指纹时需要重新登录，不再猜测原应用。断线重试参数位于“网络配置 → 高级选项”，默认折叠；原有重试行为保持不变。\n\n")
	...
	for _, component := range components {
		...
		fmt.Fprintln(&buffer, "| 字段 | 含义 | 类型 | 默认值 | 范围 / 格式 | 敏感 | 需要重启 |\n| --- | --- | --- | --- | --- | --- | --- |")
		for _, field := range component.Fields {
			...
			fmt.Fprintf(&buffer, "| `%s` | %s | `%s` | `%s` | %s | %s | %s |\n", cell(field.Name), cell(field.Title), field.Type, cell(string(value)), cell(bounds), secret, restart)
		}
		fmt.Fprintln(&buffer)
