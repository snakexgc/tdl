...
func run(path string) error {
	...
	fmt.Fprint(&buffer, "WebUI 的“系统设置 → 一键完全重置”须再次确认后执行。重置会停止服务、关闭数据库与日志，再清空应用目录内 `.tdl/`、`components/` 的内容，删除 `tdl_config.json`、旧 `config.json` 及其写入临时文件，包含全局配置、历史组件配置与密钥，以及所有命名空间的登录凭据和任务记录。自定义旧组件目录仅清理 TDL 的组件配置文件、密钥和迁移记录。目录本身保留以兼容挂载；完成后程序退出，下次启动需重新配置与登录，容器遵循自身重启策略。清理结果显示在程序控制台。\n\n")
