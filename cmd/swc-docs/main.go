...
func run(path string) error {
	...
	fmt.Fprint(&buffer, "`download.control.executors` 可设置为 `[\"aria2\",\"local\",\"http\"]`，`local_root` 留空时使用 TDL 可执行文件所在目录下的 `download` 文件夹，提交本地下载时自动创建；与当前工作目录和 `TDL_HOME` 无关。自定义目录须填写本机绝对路径。只有明确未接受任务的错误允许降级；超时、响应丢失或已有任务 ID 时停止提交。aria2 仍需启用对应模块和自动下载。列表不能为空，默认顺序为 aria2、http；保存并重启后按新顺序提交任务。只接受 local、aria2、http，不接受 internal 或 mode 字段；本地任务只使用 download.local 键空间，任务记录必须具有当前状态与版本字段，不自动补齐历史记录。\n\n")
