...
本表覆盖统一目录中的 17 个业务组件（八个静态组件和九个按资源装配的组件）。组件文件是权威来源；缺文件或缺字段使用 schema 默认值，不回退旧业务配置。生产兼容适配器仍使用普通配置 DTO；这不代表旧编排及真实环境验收已经全部完成。
...
| 字段 | 类型 | 默认值 | 范围 / 格式 | 敏感 | 需要重启 |
| --- | --- | --- | --- | --- | --- |
| `mode` | `string` | `"aria2"` | aria2 / local / internal | 否 | 是 |
...
| `secret` | `string` | `""` | — | 是 | 是 |
| `directory` | `string` | `""` | — | 否 | 是 |
| `timeout_seconds` | `int` | `30` | ≥ 1，≤ 3600 | 否 | 是 |
| `auto_download` | `bool` | `true` | — | 否 | 是 |
...
| 字段 | 类型 | 默认值 | 范围 / 格式 | 敏感 | 需要重启 |
| --- | --- | --- | --- | --- | --- |
| `mode` | `string` | `"default"` | default / clone | 否 | 是 |
| `target` | `string` | `""` | — | 否 | 是 |
| `silent` | `bool` | `false` | — | 否 | 是 |
| `dedupe_ttl_seconds` | `int` | `600` | ≥ 0，≤ 8640000 | 否 | 是 |
...
| `public_base_url` | `string` | `""` | url | 否 | 是 |
| `link_ttl_hours` | `int` | `24` | ≥ 0，≤ 876000 | 否 | 是 |
