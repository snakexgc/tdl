...
```json
{
  ...
  "threads": 4, // 单个文件同时使用的下载连接数
  "limit": 2, // 同时处理的下载数量
  "pool_size": 8, // Telegram 连接池大小
...
| `threads`              | 单个文件同时使用的下载连接数；网络越好可以适当调高，默认 4 适合多数场景 |
| `limit`                | 同时处理的下载数量；机器性能一般时保持默认即可            |
...
- 任意客户端对同一个文件发起很多分段请求时，tdl 最多只会同时处理 `threads` 个连接；多余请求会排队等待
...
Telegram 用户登录前需要先在 Web 面板填写一个用户名，tdl 会把这个用户名作为 `namespace` 保存对应的登录数据。用户名只允许英文字母，例如 `alice`。在“用户管理”里可以切换用户；切换完成后程序会自动重启，让 WebUI、机器人和监听下载都加载到新的用户数据空间。
