...
`webui.password` 设置后，直接启动程序会打开 Web 管理面板，例如访问 `http://127.0.0.1:22335`。面板使用页面登录，不再依赖浏览器弹出的 Basic Auth 窗口。面板包含用户管理、配置文件、下载管理、KV 链接管理、模块管理和检查更新。下载管理会根据 `downloader.mode` 自动切换：aria2 模式内置 AriaNg，并通过 tdl 服务端代理读取 `aria2.rpc_url` / `aria2.secret`；internal 模式显示内部任务列表，可暂停、继续和删除任务。
